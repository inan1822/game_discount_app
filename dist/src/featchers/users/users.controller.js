...
export const promoteToAdmin = async (req, res) => {
    try {
        const userId = req.params.id;
        const user = await userModel.findByIdAndUpdate(userId, { role: "admin" }, { new: true } // returns the updated user
        );
        if (!user)
            return res.status(404).json({
                status: "404",
...
export const addAddress = async (req, res) => {
    try {
        ...
        const alreadyExists = user.addresses.some(addr => addr.street === street &&
            addr.city === city &&
...
export const deleteAddress = async (req, res) => {
    try {
        ...
        const addressExists = user.addresses.find(address => String(address._id) === String(addressId));
        if (!addressExists)
            return res.status(404).json({
                status: "404",
                message: "address not found",
...
export const addToCart = async (req, res) => {
    try {
        ...
        if (existingItem) {
            await cartModel.findOneAndUpdate({ userId, "items.productId": productId }, { $inc: { "items.$.quantity": quantity } });
        }
        else {
...
export const removeFromCart = async (req, res) => {
    try {
        ...
        const itemExists = user.cart.find(item => String(item.productId) === String(productId));
        if (!itemExists)
            return res.status(404).json({
                status: "404",
